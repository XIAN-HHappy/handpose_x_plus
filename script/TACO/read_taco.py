...
if __name__ == '__main__':
    path_root = "Egocentric_RGB_Videos-HPX/"
    triangles_index = np.load("../../config/triangles_index.npy").reshape(-1,3)

    out_cnt = 0
    hand_cnt = 0
    for triplet_ in os.listdir(path_root):
        path_triplet_ = path_root + triplet_ + "/"

        for sequence_ in os.listdir(path_triplet_):
            if "_label" in sequence_:
                continue
            path_sequence_img = path_triplet_ + sequence_ + "/"
            path_sequence_label = path_triplet_ + sequence_ + "_label/"
            #---------------------------------------------------------

            for f_ in os.listdir(path_sequence_img):
                path_img = path_sequence_img + f_
                path_json = path_sequence_label + f_.replace(".jpg",".json")
                ...
                out_cnt += 1
                print("--------->>> TACO imgs_num : [{}] , hands_num: [{}]".format(out_cnt,hand_cnt))
